...
- [ ] Fix `request.json()` returning `unknown` in `api.game.save.ts` — add a typed interface and cast the body
- [ ] Fix `request.json()` returning `unknown` in `api.sync.migrate.ts` — same pattern
- [ ] Fix `tsconfig.cloudflare.json` not including `lib/sudoku/*` — the solver/types/utils files are imported by app code but not listed in the project's `include`, causing TS6307 errors
- [ ] `api.game.save.ts` has no input validation — a malformed or malicious body goes straight to the database
- [ ] `api.sync.migrate.ts` has no input validation either

## Game UX

- [ ] Completing a puzzle shows no "New Puzzle" button — user has to go Home and select difficulty again
...
- [ ] Timer keeps running while completion card is visible — it should stop the moment the puzzle is solved
- [ ] Corner notes and center notes can coexist on the same cell which is confusing — consider clearing one when the other is set
...
- [ ] No way to restart/reset a puzzle without refreshing
...
- [ ] `localStorage` progress loading on play page mount is a no-op (lines 241-252 in `play.$puzzleId.tsx` — reads but doesn't use the data)
- [ ] Settings sync failure is completely silent — `.catch(() => {})` in settings.tsx
...
- [ ] Stats page builds SVG charts client-side on every render — could memoize
...
- [ ] `formatTime()` is duplicated in 3 files (`home.tsx`, `play.$puzzleId.tsx`, `stats.tsx`)
- [ ] Auth setup (`createAuth(...)`) is repeated in every loader — could be middleware or a shared helper that also handles the try/catch
- [ ] `DIFFICULTY_RANGES` and `DIFFICULTIES` are defined in home.tsx but also needed conceptually in other places (stats, grader)
- [ ] D1 compatibility proxy in `auth.server.ts` is a fragile workaround — should track upstream better-auth fix
- [ ] No consistent error response format across API routes

## Accessibility

- [ ] Number pad buttons lack aria-labels — screen reader says "1" but not "Enter 1" or "Toggle note 1"
